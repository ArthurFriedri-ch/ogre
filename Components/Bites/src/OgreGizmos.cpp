...
// * Perspective camera box - using identity projection?
...
void addLine(const Vector3& a, const Vector3& b, const Quaternion& rot,
             ManualObject* mesh, const ColourValue& color)
{
...
void addRotatedCircle(const Quaternion& q, const Vector3& t, Real x, const Quaternion& rot,
                      ManualObject* mesh, const ColourValue& color)
{
    constexpr Real radius   = 0.22f;
    constexpr Real accuracy = 8.0f;
...
Gizmo::Gizmo(SceneManager* sceneManager, SceneNode* sceneNode, GizmoMode mode) : mMode(G_NONE)
{
    ...
    createMesh(sceneManager, "AxisGizmosMesh",
               ...
               ColourValue(0.0f, 1.0f, 0.0f, 0.50f),
               ColourValue(0.0f, 0.0f, 1.0f, 0.50f),
               ColourValue(1.0f, 1.0f, 0.0f, 0.35f),
               ColourValue(0.0f, 1.0f, 1.0f, 0.35f),
...
void Gizmo::startDrag(const Ray& startRay, const Vector3& cameraDir)
{
    ...
    const int axisCount = ((mActiveAxis & AXIS_X) ? 1 : 0) +
                          ((mActiveAxis & AXIS_Y) ? 1 : 0) +
                          ((mActiveAxis & AXIS_Z) ? 1 : 0);
    ...
    if (mDragUsePlane)
    {
        if (mMode == G_SCALE)
        {
            ...
            Ray localRay(invOri * (startRay.getOrigin() - mDragGizmoCenter),
                         invOri * startRay.getDirection());
...
void Gizmo::computeDrag(Ray& ray, const Vector3& cameraDir)
{
    ...
    if (mMode == G_TRANSLATE)
    {
        ...
        if (mDragUsePlane)
        {
            ...
            if (!(mActiveAxis & AXIS_X)) deltaLocal.x = 0;
            if (!(mActiveAxis & AXIS_Y)) deltaLocal.y = 0;
            if (!(mActiveAxis & AXIS_Z)) deltaLocal.z = 0;
    ...
    {
        const Quaternion invOri = mDragGizmoOrientation.Inverse();
        Ray localRay(invOri * (ray.getOrigin() - mDragGizmoCenter),
                     invOri * ray.getDirection());
...
void Gizmo::createMesh(SceneManager* manager, const String& name,
                       const ColourValue& xColor, const ColourValue& yColor,
                       const ColourValue& zColor,
                       const ColourValue& xyColor, const ColourValue& yzColor,
...
bool Gizmo::pickAxis(Ray& ray)
{
    ...
    const PickVolume volumes[] = {
        {axisX,  AXIS_X,  G_TRANSLATE},
        {axisY,  AXIS_Y,  G_TRANSLATE},
        {axisZ,  AXIS_Z,  G_TRANSLATE},
        {planeXY, AXIS_XY, G_TRANSLATE},
        {planeYZ, AXIS_YZ, G_TRANSLATE},
        {planeXZ, AXIS_XZ, G_TRANSLATE},
        {ringX,  AXIS_X,  G_ROTATE},
        {ringY,  AXIS_Y,  G_ROTATE},
        {ringZ,  AXIS_Z,  G_ROTATE},
    };
...
ManualObject* CameraGizmo::pickFace(float nx, float ny)
{
    ...
    float vy = ny / 0.20f;
    const Ray worldRay = mGizmoCamera->getCameraToViewportRay(vx, vy);
